"""
Tests for $ne boundary values and precision.
...
BOUNDARY_TESTS: list[ExpressionTestCase] = [
    ExpressionTestCase(
        "int32_max_as_long",
        expression={"$ne": [INT32_MAX, Int64(INT32_MAX)]},
        expected=False,
        msg="INT32_MAX equals Int64(INT32_MAX)",
    ),
    ExpressionTestCase(
        "int32_min_as_long",
        expression={"$ne": [INT32_MIN, Int64(INT32_MIN)]},
        expected=False,
        msg="INT32_MIN equals Int64(INT32_MIN)",
    ),
    ExpressionTestCase(
        "int64_max_as_dec128",
        expression={"$ne": [INT64_MAX, Decimal128(str(INT64_MAX))]},
        expected=False,
        msg="INT64_MAX equals Decimal128(INT64_MAX)",
    ),
    ExpressionTestCase(
        "int64_min_as_dec128",
        expression={"$ne": [INT64_MIN, Decimal128(str(INT64_MIN))]},
        expected=False,
...
LARGE_NUMBER_TESTS: list[ExpressionTestCase] = [
    ExpressionTestCase(
        ...
        expression={"$ne": [Int64(DOUBLE_PRECISION_LOSS), float(DOUBLE_PRECISION_LOSS)]},
        expected=True,
        msg="Int64(2^53+1) not equal to double(2^53+1) — beyond double precision",
    ),
    ExpressionTestCase(
        "exactly_representable",
        expression={"$ne": [Int64(DOUBLE_MAX_SAFE_INTEGER), float(DOUBLE_MAX_SAFE_INTEGER)]},
        expected=False,
        msg="Int64(2^53) vs double(2^53) — exactly representable",
    ),
    ExpressionTestCase(
        "dec128_preserves_precision",
        expression={"$ne": [Decimal128(str(DOUBLE_PRECISION_LOSS)), Int64(DOUBLE_PRECISION_LOSS)]},
        expected=False,
...
OVERFLOW_ADJACENCY_TESTS: list[ExpressionTestCase] = [
    ExpressionTestCase(
        ...
        expression={"$ne": [INT32_OVERFLOW, Int64(INT32_OVERFLOW)]},
        expected=False,
        msg="INT32_MAX+1 as int equals Int64(INT32_MAX+1) — promoted to long",
    ),
    ExpressionTestCase(
        "int32_underflow_as_long",
        expression={"$ne": [INT32_UNDERFLOW, Int64(INT32_UNDERFLOW)]},
        expected=False,
        msg="INT32_MIN-1 as int equals Int64(INT32_MIN-1) — promoted to long",
    ),
    ExpressionTestCase(
        "int32_overflow_vs_max",
        expression={"$ne": [INT32_OVERFLOW, INT32_MAX]},
        expected=True,
        msg="INT32_MAX+1 not equal to INT32_MAX",
    ),
    ExpressionTestCase(
        "int32_underflow_vs_min",
        expression={"$ne": [INT32_UNDERFLOW, INT32_MIN]},
        expected=True,
        msg="INT32_MIN-1 not equal to INT32_MIN",
    ),
    ExpressionTestCase(
        "int32_max_minus1",
        expression={"$ne": [INT32_MAX_MINUS_1, INT32_MAX]},
        expected=True,
        msg="INT32_MAX-1 not equal to INT32_MAX",
    ),
    ExpressionTestCase(
        "int32_min_plus1",
        expression={"$ne": [INT32_MIN_PLUS_1, INT32_MIN]},
        expected=True,
        msg="INT32_MIN+1 not equal to INT32_MIN",
    ),
    ExpressionTestCase(
        "int64_max_minus1",
        expression={"$ne": [INT64_MAX_MINUS_1, INT64_MAX]},
        expected=True,
        msg="INT64_MAX-1 not equal to INT64_MAX",
    ),
    ExpressionTestCase(
        "int64_min_plus1",
        expression={"$ne": [INT64_MIN_PLUS_1, INT64_MIN]},
        expected=True,
        msg="INT64_MIN+1 not equal to INT64_MIN",
    ),
    ExpressionTestCase(
        "int64_max_as_double",
        expression={"$ne": [INT64_MAX, float(INT64_MAX)]},
        expected=True,
        msg="INT64_MAX not equal to float(INT64_MAX) — precision loss",
    ),
    ExpressionTestCase(
        "int64_min_as_double",
        expression={"$ne": [INT64_MIN, float(INT64_MIN)]},
        expected=False,
...
def test_ne_boundary_precision(collection, test):
    """Test $ne across numeric boundary values and cross-type precision edges."""
